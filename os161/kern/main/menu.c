...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	char** data = (char**) kmalloc(nargs*sizeof(char*));
	for(i=0;i<nargs;i++){
	...
	//kprintf("progname: %s args: %s %s %s\n",progname,args[0],args[1],args[2]);
	result = runprogram(args[0], args, (int) nargs);

	//kprintf("after result = runprogram\n");
		for(i=0;i<nargs;i++){
			kfree(data[i]);
		}
	if (result) {
...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	result = runprogram(progname, (char **)args, (int) nargs);
	if (result) {
		kprintf("Running program %s failed: %s\n", args[0],
			strerror(result));
		return;
...
common_prog(int nargs, char **args)
{
	...
	//kprintf("waitthread pid %d\n", waitthread->pid);

	int *status;
	int option;
	int ret;
	return sys_waitpid(waitthread->pid, status, option, &ret);
	//kprintf("err %d\n",err);
...
{	
	...
	if (nargs != 3) {
		...
		DEBUG(DB_SYSCALL, "INVALID NUMBER OF ARGUMENTS: %u\n", nargs);
		return EINVAL;
	}

	//check it's either on or off
	if ( strcmp(args[2],"on")!=0 && strcmp(args[2],"off")!=0 ){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	//check which dbflag the user wants to change	
	switch(atoi(args[1])){
		case 1:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_LOCORE;
			else
				dbflags = dbflags & 0xFFE;
		break;
		case 2:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_SYSCALL;
			else
				dbflags = dbflags & 0xFFD;
		break;
		case 3:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_INTERRUPT;
			else
				dbflags = dbflags & 0xFFB;
		break;
		case 4:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_DEVICE;
			else
				dbflags = dbflags & 0xFF7;
		break;
		case 5:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_THREADS;
			else
				dbflags = dbflags & 0xFEF;
		break;
		case 6:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_VM;
			else
				dbflags = dbflags & 0xFDF;
		break;
		case 7:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_EXEC;
			else
				dbflags = dbflags & 0xFBF;
		break;
		case 8:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_VFS;
			else
				dbflags = dbflags & 0xF7F;
		break;
		case 9:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_SFS;
			else
				dbflags = dbflags & 0xEFF;
		break;
		case 10:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_NET;
			else
				dbflags = dbflags & 0xDFF;
		break;
		case 11:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_NETFS;
			else
				dbflags = dbflags & 0xBFF;
		break;
		case 12:
			if(strcmp(args[2],"on")==0)
				dbflags = dbflags | DB_KMALLOC;
			else
				dbflags = dbflags & 0x7FF;
		break;
		default:
			DEBUG(DB_SYSCALL, "INVALID DEBUG FLAG NUBMER: %i\n", atoi(args[1]));
			kprintf("Usage: df nr on/off\n");
			return EINVAL;
	}//end switch
...
/* Added dbflags into menus with correspond DB_types
 *
 *
 */
...
cmd_dbflags(int n, char **a)
{
	...
	//print the current value of dbflags
...
static struct {
	...
	{ "?o",		cmd_opsmenu },
	{ "?t",		cmd_testmenu },
	{ "dbflags",	cmd_dbflags },   //added dbflags 
