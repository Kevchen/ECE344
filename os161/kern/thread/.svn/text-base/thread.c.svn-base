...
///////////////////////////////////////////////////////////////////////////
void threadinit(struct thread* th){
		th->pid = -1;
		th->parent = NULL; //points to parent
		th->wait = sem_create("wait",0);
...
void plistinit(){
	//create a new node
	phead->excode = -1; //stores exitcode
	phead->exited = 0; //1 exited, 0 is not
...
void insert(struct thread* thread){
	struct pnode* newnode = kmalloc(sizeof(struct pnode));
	newnode->thread = thread; //put data in
	newnode->excode = -1; //stores exitcode
	newnode->exited = 0; //1 exited, 0 is not
	newnode->pid = thread->pid;
	newnode->waited=0;

	struct pnode* temp = phead;
...
int findexited(int pid){

	struct pnode* temp = phead;

	while(temp->next!=NULL){
		if(temp->next->pid == pid){
			return temp->next->exited;  //found thread with chosen pid, return if the process has exited
		}
		temp = temp->next;
	}
...
int findexcode(int pid){

	struct pnode* temp = phead;

	while(temp->next!=NULL){
		if(temp->next->pid == pid){
			return temp->next->excode;  //found thread with chosen pid, return excode
		}
		temp = temp->next;
	}
	//not found, return NULL
...
void setexcode(int pid, int exitcode){

	struct pnode* temp = phead;
	while(temp->next!=NULL){
		if(temp->next->pid == pid){  //found the right pnode
			temp->next->excode = exitcode;
			//temp->next->exited = 1;
			break;
		}
		temp = temp->next;
...
void thread_detach(struct thread* thread){
	...
	int i;
	for(i=0;i<MAX_CHILD;i++){
		thread->child[i]->parent = NULL;
		thread->child[i] = NULL;
	}

	//delete thread from allthread
    //delete(thread);

}
//finds child with given pid, -1 not found, return index if found
int findChildpid(pid_t pid, struct thread* thread){

	int i;
	for(i=0;i<MAX_CHILD;i++){
		if(thread->child[i] != NULL){
			if(thread->child[i]==NULL){
				//kprintf("searching thread pid %d's child pid %d, child[%d]=NULL\n",thread->pid,pid,i);
			}
			else{
			//kprintf("searching thread pid %d's child pid %d, child[%d]=%d\n",thread->pid, pid, i, thread->child[i]->pid);
			}


			if(thread->child[i]->pid == pid){
				return i; //found child with chosen pid
			}
		}
	}
	//not found, return 0
...
struct thread* findThread(pid_t pid){
	struct pnode* temp = phead;

	while(temp->next!=NULL){
		if(temp->next->pid == pid){
			return temp->next->thread;  //found thread with chosen pid, return thread*
		}
		temp = temp->next;
	}
	//not found, return NULL
...
void insertParentpid(pid_t parent, pid_t newguy){
	struct pnode* temp = phead;

	while(temp->next!=NULL){
		if(temp->next->pid == newguy){
			temp->next->parentpid = parent; //found newguy, make it aware of its parent's id
		}
		temp = temp->next;
	}
}

//1 is true, 0 is false, -1 not found
int checkChild(pid_t parent, pid_t newguy){
	struct pnode* temp = phead;

	while(temp->next!=NULL){
		if(temp->next->pid == newguy){
			if(temp->next->parentpid != parent){  //it is your parent
				return 0;
			}
			else{
				return 1;
...
//1 has waited, 0 not waited yet
int checkWaited(pid_t pid){
	struct pnode* temp = phead;

	while(temp->next!=NULL){
		if(temp->next->pid == pid){
			if(temp->next->waited == 1)  //it is your parent
				return 1;
			else
				return 0;
		}
...
thread_create(const char *name)
{
	...
	if (thread==NULL) {
		DEBUG(DB_KMALLOC, "MEMORY ALLOCATION FAIL, SIZE NEEDED: %i\n", sizeof(struct thread));
		return NULL;
	}
	...
	//kprintf("current pidlist = %d\n", pidlist);
	//kprintf("thread created\n");
/*
	kprintf("\n");
	struct pnode* temp = phead;
	while(temp->next!=NULL){
		int num = temp->next->thread->pid;
		kprintf("-%d-",num);
		temp = temp->next;
	}
	kprintf("\n");
...
exorcise(void)
{
	...
	for (i=0; i<array_getnum(zombies); i++) {
		...
		//int excode = findexcode(z->parent->pid);
		//no parent or parent has exited
		//if(z->parent == NULL || excode == 1){
		assert(z!=curthread);
		thread_destroy(z);
		//}
...
{
	phead = kmalloc(sizeof(struct pnode));
	plistinit();
	lock = sem_create("lock",0);
	...
	////////////initialize thread struct with new data member///////////////////////////
	//threadinit(me);
/*
	kprintf("\nAfter bootstrp: ");
	struct pnode* temp = phead;
	    ...
	    struct thread **ret)
{
	...
	if (newguy->t_stack==NULL) {
		kfree(newguy->t_name);
		kfree(newguy);
		DEBUG(DB_KMALLOC, "STACK ALLOCATION FAIL: MEMORY NEEDED  %u\n", STACK_SIZE);
		return ENOMEM;
	}
	...
	////////////////////connect newguy to child thread list///////////////////////////////////////////
			int i;
			for(i=0;i<MAX_CHILD;i++){
				if(curthread->child[i]==NULL){ //insert new child at this place
					curthread->child[i] = newguy;
					break;
				}
			}
			if(curthread->child[i]==NULL){
				kprintf("Insert child thread* fail\n");
			}

			///////////store parent*
			newguy->parent = curthread;
			insertParentpid(curthread->pid, newguy->pid);
...
thread_exit(void)
{
	...
	//kprintf("thread pid %d is exiting\n",curthread->pid);
	...
	if (curthread->t_vmspace) {
		...
		//free_kpages(curthread->t_vmspace->as_vbase1);
		//free_kpages(curthread->t_vmspace->as_vbase2);
	...
	//kprintf("\ncurthread %d will become ZOMBIE\n", curthread->pid);
	mi_switch(S_ZOMB);
...
void thread_signal(const void *addr)
{
	int i, result;
