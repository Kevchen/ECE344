...
int page_read(struct vnode *v, off_t offset, vaddr_t vaddr
	...
	int result, i;
	struct uio ku;
	vaddr = PADDR_TO_KVADDR(vaddr);
	if (filesize > PAGE_SIZE) {
			kprintf("ELF: warning: segment filesize > segment memsize\n");
			if(PAGE_SIZE >= filesize - faultaddress)
				filesize -=faultaddress;
			else
			filesize = PAGE_SIZE;
		}
	...
	int fillamt = PAGE_SIZE - filesize;
	if (fillamt > 0) {
		DEBUG(DB_EXEC, "ELF: Zero-filling %lu more bytes\n",
		      (unsigned long) fillamt);
		ku.uio_resid += fillamt;
		bzero(vaddr + filesize, fillamt);
	}
...
load_elf(struct vnode *v, vaddr_t *entrypoint)
{
	...
	for (i=0; i<eh.e_phnum; i++) {
		...
		kprintf("before define region i=%d\n",i);
		result = as_define_region(v,offset,curthread->t_vmspace,
					  ph.p_vaddr, ph.p_memsz,
					  ph.p_flags & PF_R,
					  ph.p_flags & PF_W,
					  ph.p_flags & PF_X);
		kprintf("defined region %d\n",i);
		if (result) {
			return result;
		}
	}
