...
#define PTEnum 1024   //number of PTE in a page
 ...
 */
struct PTE{
	struct SecondTableEntry* entry;
};

struct SecondTableEntry{
	paddr_t frame;
	int valid;
	int readOnly;
};
...
//	off_t offset2;
	struct PTE* first_table;
	struct region* regs;		//Other regions
//#endif
};
...
int               as_define_region(struct vnode* v,off_t offset,struct addrspace *as,
