...
struct thread {
	...
	struct thread* child[MAX_CHILD]; //points to child
	struct thread* parent; //points to parent
 ...
 */
int thread_exeve(const char *program, char **args);
...
/*
* structure to store all current running threads
*
*/
...
struct pnode{
	...
	int excode; //stores exitcode
	int exited; //1 exited, 0 is not
	pid_t pid;
	pid_t parentpid;
	int waited;
};
